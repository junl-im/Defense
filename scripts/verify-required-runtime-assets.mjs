...
const inventoryPath = resolve(root, 'docs/RUNTIME_ASSET_INVENTORY_v4.0.0.json');
const failures = [];
...
if (!existsSync(inventoryPath)) {
  fail('docs/RUNTIME_ASSET_INVENTORY_v4.0.0.json', 'runtime asset inventory is missing');
} else {
  ...
  for (const entry of inventory.files ?? []) {
    ...
    if (!existsSync(absolute)) {
      fail(entry.path, 'required GLB file is missing; apply the v4.0.0 full or patch package');
      continue;
    }
