...
if (approvals.length === 19 && approvals.filter((entry) => entry.status === ASSET_APPROVAL_STATES.prototype).length === 11 && approvals.filter((entry) => entry.status === ASSET_APPROVAL_STATES.review && entry.technicalReady).length === 8) pass('프로토타입 11종·기술 리뷰 8종 격리'); else fail('에셋 승인 상태');
if (audit.summary.productionApproved === 0 && audit.summary.prototypes === 11 && audit.summary.artReview === 8 && audit.summary.technicalCandidates === 8) pass('실제 GLB 감사와 리뷰 레지스트리 일치'); else fail('GLB 감사 상태');
...
if (ASSET_APPROVAL_POLICY.currentPrototypeCount === 11 && ASSET_APPROVAL_POLICY.currentReviewCount === 8) pass('승인 정책 프로토타입·리뷰 수량'); else fail('승인 정책');
