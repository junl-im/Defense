...
if (Object.keys(CURRENT_ASSET_APPROVAL).length === 14 && Object.values(CURRENT_ASSET_APPROVAL).every((entry) => entry.status === ASSET_APPROVAL_STATES.prototype && !entry.productionReady)) pass('현재 14종 프로토타입 격리'); else fail('프로토타입 승인 상태');
if (audit.summary.productionApproved === 0 && audit.summary.prototypes === 14) pass('실제 GLB 감사와 레지스트리 일치'); else fail('GLB 감사 상태');
...
if (main.includes('AAA 아트 승인 전 프로토타입') && main.includes('approval?.productionReady')) pass('게임 진단에서 로드와 승인 분리'); else fail('런타임 승인 진단');
...
if (premium.includes('preserveAuthoredStylizedPbrMaterial') && premium.includes("productionApproved ? preserveAuthoredStylizedPbrMaterial")) pass('승인 PBR 재질 보존'); else fail('승인 PBR 재질 처리');
if (ASSET_APPROVAL_POLICY.currentPrototypeCount === 14) pass('승인 정책 프로토타입 수량'); else fail('승인 정책');
