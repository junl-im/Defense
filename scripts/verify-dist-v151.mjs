...
import { assertReachableBundleMarkers, readReachableBundleText } from './lib/dist-bundle-markers.mjs';
...
const bundle = assertReachableBundleMarkers(dist, [
...
], { label: 'v151 bundled runtime' });
const reachableBundleText = readReachableBundleText(dist).text;
if (/<svg\b|image\/svg\+xml/i.test(reachableBundleText)) throw new Error('v151 character runtime introduced SVG surface');
const files = [];
...
console.log(`PASS v1.0.51+ complete Vite dist and modern character presentation markers (${report.fileCount} files, ${bundle.files.length} reachable JS files)`);
