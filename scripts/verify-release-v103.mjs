...
const checks = [
  ...
  ['runtime and service worker identities match', (main.includes(`const GAME_VERSION = '${pkg.version}'`) || (main.includes('const GAME_VERSION = PUBLIC_GAME_VERSION;') && policy.includes("from './release-identity.generated.js'"))) && ((sw.includes(`RELEASE_VERSION = '${pkg.version}'`) && sw.includes(`BUILD_ID = '${pkg.dokkaebi?.buildId}'`)) || (sw.includes("importScripts('./release-identity.generated.js')") && sw.includes('identity.releaseVersion') && sw.includes('identity.buildId')))],
  ...
  ['persistent reward duplication guard is initialized once', (constructorBlock.match(/new PersistentRewardOrchestratorV150/g) || []).length === 1 && read('src/runtime/persistent-reward-orchestrator-v150.js').includes('completedRuns = new Map()')],
