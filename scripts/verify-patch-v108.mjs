...
for (const required of ['index.html', 'src/main.js', 'src/style.css', 'public/sw.js', 'scripts/verify-release-v108.mjs']) {
  if (!changedPaths.includes(required)) failures.push(`required title exit recovery file missing: ${required}`);
}
if (manifest.baseVersion !== '1.0.7' || manifest.targetVersion !== '1.0.8') failures.push('Patch version range mismatch');
