...
const version = '1.0.6';
const baselinePath = path.join(root, 'scripts/patch-baselines/v1.0.5.json');
...
for (const required of ['index.html', 'src/main.js', '.github/workflows/deploy.yml', 'scripts/verify-release-v104.mjs', 'scripts/verify-release-v105.mjs', 'scripts/verify-release-v106.mjs']) {
  if (!changedPaths.includes(required)) throw new Error(`Required v1.0.6 recovery file is missing: ${required}`);
}
