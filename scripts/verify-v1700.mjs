...
check(Number((pkg.dokkaebi?.lineageVersion || pkg.version).split('.')[0]) >= 17, 'package version remains v17 or later');
check(Number((pkg.dokkaebi?.lineageVersion || pkg.version).split('.')[0]) >= 17 && main.includes('LEGACY_LINEAGE_VERSION'), 'runtime game lineage remains v17 or later');
