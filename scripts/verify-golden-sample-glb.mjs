...
const expectedClips = ['Idle', 'Walk', 'Run', 'Attack', 'Skill', 'Hit', 'Death'];
const nodes = new Set((json.nodes || []).map((node) => node.name));
const expectedNodes = ['Armature', 'Hips', 'Spine', 'Head', 'Arm_L', 'Hand_L', 'Arm_R', 'Hand_R', 'WeaponSocket', 'AccessorySocket', 'Leg_L', 'Foot_L', 'Leg_R', 'Foot_R'];
const failures = [];
...
check(expectedClips.every((name) => clips.includes(name)) && clips.length === 7, `golden hero clips ${clips.join(', ')}`);
check(expectedNodes.every((name) => nodes.has(name)), 'golden hero shared rig and sockets');
check((json.images || []).length >= 4 && (json.textures || []).length >= 4, 'golden hero BaseColor/Normal/ORM/Emissive textures');
check(json.asset?.extras?.styleLockId === ART_STYLE_LOCK_ID, 'golden hero style lock metadata');
