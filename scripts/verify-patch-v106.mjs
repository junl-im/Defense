...
for (const required of ['index.html', 'src/main.js', '.github/workflows/deploy.yml', 'scripts/verify-release-v104.mjs', 'scripts/verify-release-v105.mjs', 'scripts/verify-release-v106.mjs']) {
  if (!changedPaths.includes(required)) failures.push(`required recovery file missing: ${required}`);
}
if (manifest.baseVersion !== '1.0.5' || manifest.targetVersion !== '1.0.6') failures.push('Patch version range mismatch');
