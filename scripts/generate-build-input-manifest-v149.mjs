...
const outputJson = path.join(root, 'docs/generated/build-input-manifest-v149.json');
const outputMd = path.join(root, 'docs/generated/build-input-manifest-v149.md');
...
const manifest = {
  id: 'DD-BUILD-INPUT-MANIFEST-V149',
...
if (checkOnly) {
  ...
  if (currentJson !== jsonText || currentMd !== mdText) throw new Error('v149 build input manifest is stale; run npm run generate:build-input:v149');
  console.log(`PASS v1.0.49 reproducible build input manifest (${manifest.fileCount} files, ${manifest.aggregateSha256.slice(0, 12)})`);
} else {
