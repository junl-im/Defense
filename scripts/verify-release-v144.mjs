...
check(browser.includes('complete Vite bundle') && browser.includes('Page.captureScreenshot') && browser.includes('__DOKKAEBI_TEST_API__.startRun()') && browser.includes('--enable-unsafe-swiftshader') && browser.includes('Runtime.exceptionThrown') && browser.includes('Network.loadingFailed') && browser.includes('ERR_BLOCKED_BY_ADMINISTRATOR') && browser.includes('profile cleanup') && browser.includes('emulateMobile: false') && browser.includes('maximum-scale=5') && browser.includes('failedChecks') && !browser.includes('mobile-browser-recovery-v143.html'), 'complete built-game browser matrix and diagnostics');
check(/body\.mobile-hud-v23\s+#action-dock\s+button\s*\{[^}]*width\s*:\s*100%\s*!important/i.test(style), 'mobile action grid containment');
