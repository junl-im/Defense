...
const STALE_ROOT_METADATA = Object.freeze([
  'PATCH_SUMMARY.md',
  'PATCH_MANIFEST.json',
...
function removeStaleRootMetadata() {
  const removed = [];
  for (const file of STALE_ROOT_METADATA) {
    const absolute = path.join(root, file);
...
function buildOutputs(pkg, lock) {
  ...
  Object.assign(nextPkg.scripts, {
    ...
    'clean:repo-root:v151': 'node scripts/clean-obsolete-assets.mjs',
    'verify:repo-root:v151': 'node scripts/verify-repository-root-v151.mjs',
    'prepare:repo-root:v151': 'npm run bootstrap:identity:v151 && npm run clean:repo-root:v151 && npm run verify:repo-root:v151',
    'verify:ci-root-cleanup:v151': 'node scripts/verify-ci-root-cleanup-v151.mjs',
    'verify:ci-source:v151': 'node scripts/verify-ci-source-revision-v151.mjs',
    'verify:long-session-hotfix:v151': 'node scripts/verify-long-session-hotfix-v151.mjs',
    'verify:enemy-material:v151': 'node scripts/verify-enemy-material-lifecycle-v151.mjs',
    'verify:release:v151': 'npm run verify:repo-root:v151 && npm run verify:identity:v151 && npm run verify:build-input:v151 && npm run verify:foundation:v150:v151 && npm run verify:character:v151 && npm run verify:ci-root-cleanup:v151 && npm run verify:ci-source:v151 && npm run verify:enemy-material:v151 && npm run verify:long-session-hotfix:v151 && node scripts/verify-release-v151.mjs',
    'verify:dist:v151': 'node scripts/verify-dist-v151.mjs',
    'verify:dist:all': 'npm run verify:ci-source:v151 && node scripts/verify-dist-chain-v140.mjs',
    ...
    'verify:ci': 'npm run prepare:repo-root:v151 && npm run sync:generated:ci && npm run verify',
    preverify: 'npm run prepare:repo-root:v151 && npm run hygiene:check && npm run verify:identity:v151',
    prebuild: 'npm run prepare:repo-root:v151 && npm run hygiene:check && npm run verify:identity:v151'
  });
