...
function buildOutputs(pkg, lock) {
  ...
  Object.assign(nextPkg.scripts, {
    ...
    'verify:release:v151': 'npm run verify:repo-root:v151 && npm run verify:identity:v151 && npm run verify:build-input:v151 && npm run verify:foundation:v150:v151 && npm run verify:character:v151 && npm run verify:ci-root-cleanup:v151 && npm run verify:long-session-hotfix:v151 && node scripts/verify-release-v151.mjs',
