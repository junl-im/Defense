...
const patch=Number(version.releaseVersion.split('.')[2]); if (!version.releaseVersion.startsWith('1.0.') || patch < 41 || version.buildId !== `b24.${patch}`) throw new Error('v1.0.41+ dist identity mismatch');
...
console.log(`PASS v1.0.41+ dist enables state-backed map touch and excludes legacy loading presentation under ${version.releaseVersion}`);
