...
const checks = [
  ['package retains v23 boot recovery lineage', /^23\.(?:0\.[12]|[1-9]\.\d+)$/.test(pkg.version)],
  ['game retains v23 boot recovery lineage', /GAME_VERSION = '23\.(?:0\.[12]|[1-9]\.\d+)'/.test(main)],
  ...
  ['static build uses recovery loader', /static-bootstrap\.js\?v=23\.\d+\.\d+/.test(staticBuilder) && staticBuilder.includes('node_modules/three')],
  ...
  ['title art has hotfix cache revision', (html.includes('boot-recovery-v2301') || html.includes('clean-foundation-v2302') || html.includes('native-input-v2310'))],
  ['CI verifies self-contained Vite output', (workflow.includes('verify-production-bundle-v2301.mjs') || workflow.includes('verify-production-bundle-v2302.mjs') || workflow.includes('verify-production-bundle-v2310.mjs')) && fs.existsSync('scripts/verify-production-bundle-v2301.mjs')],
