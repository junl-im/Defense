...
const checks = [
  ['package retains v23 boot recovery lineage', /^23\.0\.[12]$/.test(pkg.version)],
  ['game retains v23 boot recovery lineage', /GAME_VERSION = '23\.0\.[12]'/.test(main)],
  ['asset revision retains boot recovery lineage', /ASSET_REVISION = '23\.0\.[12]'/.test(catalog)],
  ['service worker retains boot recovery lineage', /VERSION = '23\.0\.[12]'/.test(sw)],
  ...
  ['static build uses recovery loader', staticBuilder.includes('static-bootstrap.js?v=23.0.2') && staticBuilder.includes('node_modules/three')],
  ...
  ['title art has hotfix cache revision', (html.includes('boot-recovery-v2301') || html.includes('clean-foundation-v2302'))],
  ['CI verifies self-contained Vite output', (workflow.includes('verify-production-bundle-v2301.mjs') || workflow.includes('verify-production-bundle-v2302.mjs')) && fs.existsSync('scripts/verify-production-bundle-v2301.mjs')],
