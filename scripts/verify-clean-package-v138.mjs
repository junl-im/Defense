...
const stage = process.argv[2] ? path.resolve(process.argv[2]) : path.join(root, 'logs/package/1.0.38', packageName);
const failures = [];
...
if (fs.existsSync(stage)) {
  ...
  check(pkg.version === '1.0.38' && pkg.dokkaebi?.buildId === 'b24.38', 'package identity');
  check(fs.existsSync(path.join(stage, 'scripts/lib/verify-dist-presentation-surface.mjs')), 'presentation verifier preserved');
...
console.log(`PASS clean package v1.0.38 verified at ${stage}`);
