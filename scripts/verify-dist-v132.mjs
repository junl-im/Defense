...
const revision = Number(String(version.buildId || '').split('.')[1]);
if (!String(version.releaseVersion || '').startsWith('1.0.') || patch < 32 || !String(version.buildId || '').startsWith('b24.') || revision < 32) throw new Error('v1.0.32+ dist identity mismatch');
