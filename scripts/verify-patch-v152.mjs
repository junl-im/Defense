...
if (manifest.id !== 'DD-DIRECT-OVERLAY-PATCH-V152-R1' || manifest.repairRevision !== 1 || manifest.baseVersion !== '1.0.52' || !manifest.compatibleBaseVersions?.includes('1.0.51') || !manifest.compatibleBaseVersions?.includes('1.0.52') || manifest.targetVersion !== '1.0.52' || manifest.buildId !== 'b24.52' || manifest.applyMode !== 'overlay-directory-to-project-root') throw new Error('v152 R1 patch identity mismatch');
...
for (const required of ['package.json', '.github/workflows/deploy.yml', 'scripts/verify-repository-root-v152.mjs', 'scripts/verify-project.mjs', 'scripts/verify-performance-trend-v145.mjs', 'scripts/verify-performance-guard-v148.mjs', 'src/main.js', 'src/runtime/character-action-timing-v152.js', 'src/runtime/character-presentation-budget-v152.js', 'src/engine/gpu-frame-timer-v152.js', 'PROJECT_HANDOFF.md']) if (!paths.has(required)) throw new Error(`v152 patch contract file missing ${required}`);
console.log(`PASS v1.0.52 CI hotfix R1 direct-overlay patch with SHA-256 verification (${paths.size} files)`);
