...
const checks = [
  ...
  ['game version remains v21 or later', /GAME_VERSION = '(?:21|22)\.0\.0'/.test(main)],
  ['engine version remains 18 or later', /ENGINE_VERSION = '(?:18|19)\.0\.0'/.test(read('src/engine/engine-config.js'))],
  ['save schema remains 19 or later', /SAVE_SCHEMA_VERSION = (?:19|20)/.test(read('src/runtime/save-schema.js'))],
  ['service worker remains v21 or later', /VERSION = '(?:21|22)\.0\.0'/.test(read('public/sw.js'))],
  ['asset presence module wired', main.includes('new AssetPresenceEnforcer') && fs.existsSync('src/runtime/asset-presence-enforcer.js')],
  ['mobile HUD v21 lineage retained', (main.includes('MobileHudDirectorV21') || main.includes('MobileHudDirectorV22')) && (css.includes('mobile-hud-v21-emergency') || css.includes('mobile-hud-v22-emergency'))],
