...
const checks = [
  ...
  ['title uses approved raster art only', html.includes('title-mascot-v17.webp') && html.includes('title-bg-desktop-v17.webp') && html.includes('title-bg-mobile-v17.webp') && !html.includes('<svg') && (html.includes('./assets/ip-v10/presentation/ui/button_start.png') || html.includes('title-touch-start-v107'))],
  ...
  ['start label remains state-aware without replacing title art', main.includes("startLabel.textContent = '수호대 출전 준비 중...'") && (main.includes("startLabel.textContent = '달빛 장터 수호 준비 완료'") || main.includes("startLabel.textContent = 'TOUCH TO START'")) && !main.includes("ui.start.textContent = '수호 시작'")],
