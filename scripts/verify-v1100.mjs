...
check(/const GAME_VERSION = '(?:11|12|13)\.0\.0'/.test(main), 'runtime game version remains v11 or later');
check(Number(ENGINE_VERSION.split('.')[0]) >= 9, 'engine version remains 9.0.0 or later');
check(/ASSET_REVISION = '(?:11|12|13)\.0\.0'/.test(catalog), 'asset revision remains v11 or later');
...
check((html.includes('SOVEREIGN ASSEMBLY') || html.includes('GOLDEN DOMINION')) && html.includes('id="council-options"') && html.includes('id="boss-break-progress"') && (html.includes('v11.0.0') || html.includes('v12.0.0') || html.includes('v13.0.0')), 'v11 systems retained in current title and UI');
