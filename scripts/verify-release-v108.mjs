...
const modernEntryV141 = main.includes('run-entry-pending-v141') && main.includes('run-entry-deferred-assets-v141') && !main.includes("ui.loading?.classList.add('visible'");
const modernDistV141 = distMain.includes('run-entry-pending-v141') && !distMain.includes('run-entry-loading-v108');
...
const checks = [
  ...
  ['title start transition preserves a painted presentation before world rebuild', main.includes('async startRunFromTitle') && (main.includes("ui.loading?.classList.add('visible', 'run-entry-loading-v108')") || modernEntryV141) && main.includes('await this.waitForUiPaint(2, 900)') && main.indexOf('await this.waitForUiPaint(2, 900)') < main.indexOf('this.startRun({ reuseSeed });')],
  ['stuck title copy is removed and transition copy is user-facing', !main.includes('수호대 출전 준비 중...') && main.includes('월문을 여는 중...') && (main.includes('수호대를 전장으로 부르는 중...') || modernEntryV141)],
