...
check(Number((pkg.dokkaebi?.lineageVersion || pkg.version).split('.')[0]) >= 15, 'package version remains v15 or later');
check(Number((pkg.dokkaebi?.lineageVersion || pkg.version).split('.')[0]) >= 15 && main.includes('LEGACY_LINEAGE_VERSION'), 'runtime game lineage remains v15 or later');
