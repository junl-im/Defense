...
const outputJson = path.join(root, 'docs/generated/build-input-manifest-v152.json');
const outputMd = path.join(root, 'docs/generated/build-input-manifest-v152.md');
...
const manifest = {
  id: 'DD-BUILD-INPUT-MANIFEST-V152',
...
if (checkOnly) {
  ...
  if (currentJson !== jsonText || currentMd !== mdText) throw new Error('v152 build input manifest is stale; run npm run generate:build-input:v152');
  console.log(`PASS v1.0.52 reproducible build input manifest (${manifest.fileCount} files, ${manifest.aggregateSha256.slice(0, 12)})`);
} else {
