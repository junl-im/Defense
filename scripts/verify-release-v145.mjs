...
const harnessContract = 'scripts/verify-long-session-harness-v145.mjs';
...
check(assurance.includes('DD-LONG-SESSION-ASSURANCE-V145') && assurance.includes('maxHeapSlopeMBPer10Waves') && assurance.includes('maxFrameSlopeMsPer10Waves') && assurance.includes('maxCalibratedFrameP95Ratio') && assurance.includes('maxCalibratedLongTaskRateDelta') && assurance.includes('software-renderer-calibrated') && assurance.includes('measurementCoverage') && assurance.includes('frameLongTaskRate') && assurance.includes('normalizeLongSessionBrowserSampleV145') && assurance.includes('longTaskMsGrowth') && assurance.includes('contextExercise'), 'long-session hardware budget, software-renderer calibration, coverage, and measured long-task evidence');
check(main.includes("LongSessionAssuranceV145") && main.includes('prepareLongSessionV145') && main.includes('advanceLongSessionWaveV145') && main.includes('getLongSessionEnvironmentV145') && main.includes('software-renderer-ci') && main.includes('stabilizeLongSessionMeasurementV145') && main.includes('measureFrameWindowV145') && main.includes('requestedSamples') && main.includes('measuredFrames') && main.includes('longTasksDelta') && main.includes('warmupFrames') && main.includes('exerciseWebGLRecoveryV145') && main.includes('finishLongSessionV145'), 'complete game long-session environment calibration API and stable measurement window');
check(browser.includes('V145-CI-100-WAVES') && browser.includes('targetWaves:100') && browser.includes('wave===50') && browser.includes('exerciseWebGLRecoveryV145') && browser.includes('--js-flags=--expose-gc') && browser.includes('--disable-background-timer-throttling') && browser.includes('V145_DEVICE_SCALE_FACTOR') && browser.includes('stabilizeLongSessionMeasurementV145') && browser.includes('measureFrameWindowV145(24,{warmupFrames:3})') && browser.includes('frameLongTaskRate') && browser.includes('measurementMode') && browser.includes('failedChecks') && browser.includes('worstFrames') && !browser.includes("if(typeof gc==='function') gc()") && browser.includes('Page.captureScreenshot') && browser.includes('Runtime.exceptionThrown') && browser.includes('Network.loadingFailed'), '100-wave Chromium harness, software-renderer-safe scheduling, controlled-GC isolation, and diagnostics');
...
check(fs.existsSync(path.join(root, harnessContract)), 'long-session harness contract verifier');
...
const harnessRun = spawnSync(process.execPath, [path.join(root, harnessContract)], { cwd: root, encoding: 'utf8' });
process.stdout.write(harnessRun.stdout || '');
process.stderr.write(harnessRun.stderr || '');
if (harnessRun.error || harnessRun.status !== 0) process.exit(harnessRun.status || 1);
