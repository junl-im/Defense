...
check(residency.id === 'DD-ASSET-RESIDENCY-V145' && residency.releaseVersion === '1.0.45' && residency.integrity?.allClassifiedOnce === true && residency.totals?.boot?.count === 13 && residency.totals?.deferred?.count === 40 && residency.edges?.length >= residency.assets?.length, 'startup/deferred residency and explicit reachability');
