...
const visual = read('src/runtime/combat-visual-director-v110.js');
...
const distVisual = currentDist ? read('dist/src/runtime/combat-visual-director-v110.js') : '';
...
const checks = [
  ...
  ['static builder emits the current package identity', buildStatic.includes(`const version = '${version}'`) && buildStatic.includes(`const buildId = '${buildId}'`) && buildStatic.includes(`src/bootstrap.js?v=${version}-${buildId}`)],
  ['current dist preserves directional combat visual and desktop HUD shell when present', !currentDist || (distMain.includes('CombatVisualDirectorV110') && distVisual.includes('directional-action-projection-v110') && distCss.includes('--desktop-hud-top-v110'))],
