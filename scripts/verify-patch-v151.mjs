...
if (manifest.id !== 'DD-DIRECT-OVERLAY-PATCH-V151' || manifest.baseVersion !== '1.0.50' || manifest.targetVersion !== '1.0.51' || manifest.buildId !== 'b24.51' || manifest.applyMode !== 'direct-root-overlay' || manifest.repairRevision !== 5 || manifest.archiveRootLayout !== 'flat' || manifest.repositoryRootGuard !== 'verify:repo-root:v151') throw new Error('v151 patch identity mismatch');
...
for (const required of ['package.json', '.github/workflows/deploy.yml', 'scripts/clean-obsolete-assets.mjs', 'scripts/root-output-policy.mjs', 'scripts/verify-repository-root-v151.mjs', 'scripts/verify-ci-root-cleanup-v151.mjs', 'scripts/run-release-assurance-v146.mjs', 'scripts/verify-dist-v146.mjs', 'scripts/verify-release-v146.mjs', 'scripts/verify-long-session-hotfix-v151.mjs', 'scripts/verify-ci-source-revision-v151.mjs', 'docs/generated/ci-source-revision-v151.json', 'src/main.js', 'src/runtime/character-presentation-director-v151.js', 'src/runtime/long-session-load-profile-v151.js', 'src/engine/character-material-enhancer-v151.js', 'docs/PATCH_PROVENANCE_v1.0.51.json']) if (!paths.has(required)) throw new Error(`v151 patch contract file missing ${required}`);
