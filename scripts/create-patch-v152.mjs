...
const out = path.join(root, 'logs/patch/1.0.52-r10');
...
for (const file of V152_PATCH_FILES) {
  ...
  if (!fs.existsSync(source) || !fs.statSync(source).isFile()) throw new Error(`v152 R10 patch source missing: ${file}`);
...
const manifest = {
  id: 'DD-PROJECT-ROOT-PATCH-V152-R10',
  repairRevision: 10,
  baseVersion: '1.0.52',
  ...
  baseZipSha256: 'b62a72e1724d8462798d8daff2658eaa442f82c7b9d5d70b2480396845580de4',
...
fs.writeFileSync(path.join(out, 'PATCH_MANIFEST_R10.json'), `${JSON.stringify(manifest, null, 2)}\n`);
fs.writeFileSync(path.join(out, 'DELETE_PATHS_R10.txt'), `${V152_DELETED_PATHS.join('\n')}\n`);
