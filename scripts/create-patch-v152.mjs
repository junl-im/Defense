...
const out = path.join(root, 'logs/patch/1.0.52-r9');
...
for (const file of V152_PATCH_FILES) {
  ...
  if (!fs.existsSync(source) || !fs.statSync(source).isFile()) throw new Error(`v152 R9 patch source missing: ${file}`);
...
const manifest = {
  id: 'DD-PROJECT-ROOT-PATCH-V152-R9',
  repairRevision: 9,
  baseVersion: '1.0.52',
  ...
  baseZipSha256: 'dcc05b0da1e08153f1703c2fcc244f8dc4b68e253d775c0cffa099a24ca426c0',
...
fs.writeFileSync(path.join(out, 'PATCH_MANIFEST_R9.json'), `${JSON.stringify(manifest, null, 2)}\n`);
fs.writeFileSync(path.join(out, 'DELETE_PATHS_R9.txt'), `${V152_DELETED_PATHS.join('\n')}\n`);
