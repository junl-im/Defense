...
const out = path.join(root, 'logs/patch/1.0.52-r4');
...
for (const file of V152_PATCH_FILES) {
  ...
  if (!fs.existsSync(source) || !fs.statSync(source).isFile()) throw new Error(`v152 R4 patch source missing: ${file}`);
...
const manifest = {
  id: 'DD-PROJECT-ROOT-PATCH-V152-R4',
  repairRevision: 4,
  baseVersion: '1.0.52',
  ...
  baseZipSha256: '971af9d5edb0bd91cbe22b25ef5f16c2149c81c33011725b8f929ee8ff231e5c',
...
fs.writeFileSync(path.join(out, 'PATCH_MANIFEST_R4.json'), `${JSON.stringify(manifest, null, 2)}\n`);
fs.writeFileSync(path.join(out, 'DELETE_PATHS_R4.txt'), `${V152_DELETED_PATHS.join('\n')}\n`);
