...
const out = path.join(root, 'logs/patch/1.0.52-r3');
...
for (const file of V152_PATCH_FILES) {
  ...
  if (!fs.existsSync(source) || !fs.statSync(source).isFile()) throw new Error(`v152 R3 patch source missing: ${file}`);
...
const manifest = {
  id: 'DD-PROJECT-ROOT-PATCH-V152-R3',
  repairRevision: 3,
  baseVersion: '1.0.52',
  ...
  baseZipSha256: 'ff57ca022912bd346fee74ea2497d9d1181dd0bf53bb41849940a47e76f4d066',
...
fs.writeFileSync(path.join(out, 'PATCH_MANIFEST_R3.json'), `${JSON.stringify(manifest, null, 2)}\n`);
fs.writeFileSync(path.join(out, 'DELETE_PATHS_R3.txt'), `${V152_DELETED_PATHS.join('\n')}\n`);
