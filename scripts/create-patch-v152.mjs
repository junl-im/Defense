...
const out = path.join(root, 'logs/patch/1.0.52-r11');
...
for (const file of V152_PATCH_FILES) {
  ...
  if (!fs.existsSync(source) || !fs.statSync(source).isFile()) throw new Error(`v152 R11 patch source missing: ${file}`);
...
const manifest = {
  id: 'DD-PROJECT-ROOT-PATCH-V152-R11',
  repairRevision: 11,
  baseVersion: '1.0.52',
  ...
  baseZipSha256: '9cab66674986f5861e98f334e7a983339ccaf1b717bf4bd12d5214a8d5d51b84',
...
fs.writeFileSync(path.join(out, 'PATCH_MANIFEST_R11.json'), `${JSON.stringify(manifest, null, 2)}\n`);
fs.writeFileSync(path.join(out, 'DELETE_PATHS_R11.txt'), `${V152_DELETED_PATHS.join('\n')}\n`);
