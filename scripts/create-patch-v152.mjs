...
const out = path.join(root, 'logs/patch/1.0.52-r12');
...
for (const file of V152_PATCH_FILES) {
  ...
  if (!fs.existsSync(source) || !fs.statSync(source).isFile()) throw new Error(`v152 R12 patch source missing: ${file}`);
...
const manifest = {
  id: 'DD-PROJECT-ROOT-PATCH-V152-R12',
  repairRevision: 12,
  baseVersion: '1.0.52',
  ...
  baseZipSha256: 'f10095d9887006b7ab50ab7c6b19a58f5e32cd0a47de85f31f3a36d1e7385960',
...
fs.writeFileSync(path.join(out, 'PATCH_MANIFEST_R12.json'), `${JSON.stringify(manifest, null, 2)}\n`);
fs.writeFileSync(path.join(out, 'DELETE_PATHS_R12.txt'), `${V152_DELETED_PATHS.join('\n')}\n`);
