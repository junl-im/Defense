...
const out = path.join(root, 'logs/patch/1.0.52-r6');
...
for (const file of V152_PATCH_FILES) {
  ...
  if (!fs.existsSync(source) || !fs.statSync(source).isFile()) throw new Error(`v152 R6 patch source missing: ${file}`);
...
const manifest = {
  id: 'DD-PROJECT-ROOT-PATCH-V152-R6',
  repairRevision: 6,
  baseVersion: '1.0.52',
  ...
  baseZipSha256: '764a5d7812b36004f63e6fb20cf35fcbbfb6c64ad10eba5fb4d7c0901a37e173',
...
fs.writeFileSync(path.join(out, 'PATCH_MANIFEST_R6.json'), `${JSON.stringify(manifest, null, 2)}\n`);
fs.writeFileSync(path.join(out, 'DELETE_PATHS_R6.txt'), `${V152_DELETED_PATHS.join('\n')}\n`);
