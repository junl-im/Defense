...
const out = path.join(root, 'logs/patch/1.0.52-r8');
...
for (const file of V152_PATCH_FILES) {
  ...
  if (!fs.existsSync(source) || !fs.statSync(source).isFile()) throw new Error(`v152 R8 patch source missing: ${file}`);
...
const manifest = {
  id: 'DD-PROJECT-ROOT-PATCH-V152-R8',
  repairRevision: 8,
  baseVersion: '1.0.52',
  ...
  baseZipSha256: 'f41a9657fc3690729c3fa843db67c23d19d2c3a9e3f6083d4e129c7bded5ead3',
...
fs.writeFileSync(path.join(out, 'PATCH_MANIFEST_R8.json'), `${JSON.stringify(manifest, null, 2)}\n`);
fs.writeFileSync(path.join(out, 'DELETE_PATHS_R8.txt'), `${V152_DELETED_PATHS.join('\n')}\n`);
