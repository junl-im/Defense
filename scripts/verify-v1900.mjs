...
check(Number((pkg.dokkaebi?.lineageVersion || pkg.version).split('.')[0]) >= 19, 'package version 19.0.0 or later');
check(Number((pkg.dokkaebi?.lineageVersion || pkg.version).split('.')[0]) >= 19 && main.includes('LEGACY_LINEAGE_VERSION'), 'runtime game lineage 19.0.0 or later');
...
check(sw.includes(`const RELEASE_VERSION = '${pkg.dokkaebi?.releaseVersion || pkg.version}'`) && sw.includes(`const BUILD_ID = '${pkg.dokkaebi?.buildId}'`), 'service worker preserves release and monotonic build identity');
check(sw.includes('const CACHE_NAME = `${CACHE_PREFIX}${BUILD_ID}`'), 'service worker uses a build-versioned shell cache');
...
check(html.includes(`const RELEASE_VERSION = '${pkg.dokkaebi?.releaseVersion || pkg.version}'`) && html.includes(`const BUILD_ID = '${pkg.dokkaebi?.buildId}'`), 'boot coordinator preserves release and build identity');
