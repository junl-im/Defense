...
check(pkg.version === '19.0.0', 'package version 19.0.0');
check(main.includes("const GAME_VERSION = '19.0.0'"), 'runtime game version 19.0.0');
check(ENGINE_VERSION === '16.0.0', 'engine version 16.0.0');
check(SAVE_SCHEMA_VERSION === 17, 'save schema version 17');
...
check(sw.includes("const VERSION = '19.0.0'"), 'service worker version 19.0.0');
...
check(html.includes("const VERSION = '19.0.0'"), 'boot coordinator version 19.0.0');
...
check(consoleSource.includes('BROWSER RELIABILITY LAB v19') && consoleSource.includes('BROWSER LAB'), 'production console exposes browser reliability diagnostics');
