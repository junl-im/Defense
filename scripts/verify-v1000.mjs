...
check(/const GAME_VERSION = '(?:10|11|12|13)\.0\.0'/.test(main), 'runtime retains v10 lineage or later');
check(Number(ENGINE_VERSION.split('.')[0]) >= 8, 'engine version remains 8.0.0 or later');
check(/ASSET_REVISION = '(?:10|11|12|13)\.0\.0'/.test(assetCatalog), 'asset revision remains v10 or later');
...
check((html.includes('asset-library-v13.html') || html.includes('asset-library-v10.html')) && (html.includes('5 CLASSES') || html.includes('COUNCIL 15') || html.includes('5직업') || html.includes('15개 결속')) && (html.includes('ALPHA 40') || html.includes('BREAK SYSTEM') || html.includes('RUNTIME SLICE 6/6')) && (html.includes('v10.0.0') || html.includes('v11.0.0') || html.includes('v12.0.0') || html.includes('v13.0.0')), 'title screen exposes v10 forge and roster');
...
check((consoleSource.includes('IP_ASSET_LIBRARY_V10') || consoleSource.includes('IP_ASSET_LIBRARY_V13')) && consoleSource.includes('HERO ROSTER') && consoleSource.includes('ASSET FORGE'), 'production console v10 diagnostics');
