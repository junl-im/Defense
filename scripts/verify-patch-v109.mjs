...
if (changedPaths.some((file) => file.startsWith('src/assets/') || file.startsWith('public/assets/') || file.startsWith('dist/src/assets/') || file.startsWith('dist/assets/') || file === 'src/art-style-tokens.js' || file === 'dist/src/art-style-tokens.js' || file === 'docs/ABSOLUTE_ART_BIBLE_v2.0.md')) failures.push('Art Bible protected asset path changed');
for (const required of ['index.html', 'src/main.js', 'src/style.css', 'src/engine/asset-catalog.js', 'src/runtime/combat-art-skin-v109.js', 'src/ui-layout-manager.js', 'public/sw.js', 'scripts/verify-release-v109.mjs']) {
  if (!changedPaths.includes(required)) failures.push(`required combat art or desktop HUD file missing: ${required}`);
}
if (manifest.baseVersion !== '1.0.8' || manifest.targetVersion !== '1.0.9') failures.push('Patch version range mismatch');
