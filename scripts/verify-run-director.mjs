...
assert.equal(MOON_OMENS.length, 5, 'moon omen count');
assert.equal(ELITE_AFFIXES.length, 4, 'elite affix count');
assert.notEqual(selectMoonOmen('harvest', () => 0).id, 'harvest', 'omen should avoid immediate repeat');
assert.equal(rollEliteAffix(2, MOON_OMENS[0], () => 0), null, 'no elites before wave 3');
