...
const checks = [
  ...
  ['runtime game version matches package', main.includes(`const GAME_VERSION = '${pkg.version}'`) || (main.includes('const GAME_VERSION = PUBLIC_GAME_VERSION;') && policy.includes("from './release-identity.generated.js'"))],
  ['service worker release/build match', (sw.includes(`RELEASE_VERSION = '${pkg.version}'`) && sw.includes(`BUILD_ID = '${pkg.dokkaebi?.buildId}'`)) || (sw.includes("importScripts('./release-identity.generated.js')") && sw.includes('identity.releaseVersion') && sw.includes('identity.buildId'))],
