...
const requireBrowser = process.env.REQUIRE_BROWSER_V145 === '1';
const reportPath = path.join(root, 'logs/qa/v145/long-session-report.json');
const screenshotPath = path.join(root, 'logs/qa/v145/long-session-wave-100.png');
...
const failOrSkip = (message) => {
  ...
  console.log(`SKIP v1.0.45 long-session browser assurance: ${message}`);
  process.exit(0);
};
...
const profile = fs.mkdtempSync(path.join(os.tmpdir(), 'dokkaebi-v145-long-session-'));
...
try {
  ...
  diagnostics.navigation = await client.send('Page.navigate', { url: `http://127.0.0.1:${port}/?qa=v145&longSession=100` });
  ...
  sessionReport = await evaluate(client, `(async()=>{
    ...
    const progress=[];
    await api.prepareLongSessionV145({targetWaves:100,seed:'V145-CI-100-WAVES'});
    for(let wave=1;wave<=100;wave+=1){
      if(Date.now()>deadline) throw new Error('v145 long-session deadline exceeded');
      const snapshot=await api.advanceLongSessionWaveV145();
      if(snapshot.currentWave!==wave) throw new Error('v145 wave mismatch '+snapshot.currentWave+' != '+wave);
      if(wave===50){
        const recovery=await api.exerciseWebGLRecoveryV145();
        if(!recovery.supported||!recovery.lost||!recovery.restored) throw new Error('v145 WebGL recovery failed: '+JSON.stringify(recovery));
      }
      if(wave%5===0){
        if(typeof gc==='function') gc();
        const frameWindow=await api.measureFrameWindowV145(10);
        const sample=api.recordLongSessionSampleV145(frameWindow);
        progress.push({wave:sample.wave,frameP95Ms:sample.frameP95Ms,heapUsedMB:sample.heapUsedMB,textures:sample.textures,geometries:sample.geometries});
      }
    }
  ...
  if (!sessionReport?.report?.passed) throw new Error(`v145 long-session checks failed: ${JSON.stringify(sessionReport?.report?.checks || {})}`);
  ...
  if (meaningfulFailures.length) throw new Error(`v145 network failures captured: ${meaningfulFailures.length}`);
} catch (error) {
...
const report = {
  id: 'DD-LONG-SESSION-BROWSER-V145',
  releaseVersion: '1.0.45',
  buildTarget: 'complete-vite-dist',
  targetWaves: 100,
  seed: 'V145-CI-100-WAVES',
  chromium: command,
...
for (let attempt = 0; attempt < 6; attempt += 1) {
  try { fs.rmSync(profile, { recursive: true, force: true }); break; }
    ...
    if (attempt === 5) console.warn(`WARN v145 Chromium profile cleanup: ${error.code || error.message}`);
    else await sleep(120 * (attempt + 1));
  }
}
if (!report.passed) {
  console.error(`FAIL v1.0.45 100-wave long-session assurance: ${fatalError || 'unknown failure'}`);
  process.exit(1);
}
console.log('PASS v1.0.45 100-wave long-session assurance with frame, heap, renderer, and WebGL recovery evidence');
