...
const packageName = 'DokkaebiLuckDefense3D_FULL_v1.0.39_CI_PORTABILITY_VERIFIED';
const stage = process.argv[2] ? path.resolve(process.argv[2]) : path.join(root, 'logs/package/1.0.39', packageName);
const failures = [];
...
if (fs.existsSync(stage)) {
  ...
  check(pkg.version === '1.0.39' && pkg.dokkaebi?.buildId === 'b24.39', 'package identity');
  check(fs.existsSync(path.join(stage, 'scripts/lib/verify-dist-v134-foundation.mjs')), 'portable v134 verifier preserved');
...
console.log(`PASS clean package v1.0.39 verified at ${stage}`);
