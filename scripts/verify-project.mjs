...
if (pkg.version === '3.5.0') pass('package version 3.5.0');
...
if (main.includes("const GAME_VERSION = '3.5.0'")) pass('런타임 version 3.5.0');
...
if (html.includes('class="title-version"') && html.includes('>v3.5.0</small>') && !html.includes('3대 월식 보스, 오늘의 원정')) pass('타이틀은 버전만 표시');
...
for (const legacy of [
  ...
  if (!existsSync(resolve(root, legacy))) pass(`구버전 중복 번들 제거 ${legacy}`);
  else fail(`구버전 중복 번들 잔존: ${legacy}`);
}
...
if (engineConfig.includes("ENGINE_VERSION = '2.5.0'") && engineConfig.includes('unitTriangles: 10000') && engineConfig.includes('enemyTriangles: 3200') && engineConfig.includes('bossTriangles: 9000')) pass('엔진 버전과 프로토타입 런타임 폴리곤 예산');
