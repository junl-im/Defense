...
const outputJson = path.join(root, 'docs/generated/build-input-manifest-v151.json');
const outputMd = path.join(root, 'docs/generated/build-input-manifest-v151.md');
...
const manifest = {
  id: 'DD-BUILD-INPUT-MANIFEST-V151',
...
if (checkOnly) {
  ...
  if (currentJson !== jsonText || currentMd !== mdText) throw new Error('v151 build input manifest is stale; run npm run generate:build-input:v151');
  console.log(`PASS v1.0.51 reproducible build input manifest (${manifest.fileCount} files, ${manifest.aggregateSha256.slice(0, 12)})`);
} else {
