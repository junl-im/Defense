...
const sourcePreflight = spawnSync(process.execPath, [path.join(root, 'scripts/verify-ci-source-revision-v152.mjs')], { cwd: root, env: process.env, encoding: 'utf8' });
process.stdout.write(sourcePreflight.stdout || ''); process.stderr.write(sourcePreflight.stderr || '');
if (sourcePreflight.status !== 0) throw new Error(`DD-V152-EVENT-TIMING-RUNTIME-GUARD preflight failed (${sourcePreflight.status})`);
