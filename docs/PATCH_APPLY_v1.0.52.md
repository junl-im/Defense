...
R12는 R11 또는 이전 v1.0.52 프로젝트 루트에 직접 덮어쓰는 CI 핫픽스다. 게임 버전과 빌드 ID는 `1.0.52 / b24.52`로 유지된다.

1. 기존 프로젝트를 별도 위치에 백업한다.
2. R12 패치 ZIP의 내용을 프로젝트 루트에 직접 덮어쓴다.
3. `DELETE_PATHS_v1.0.52_R12.txt`에 적힌 생성물·과거 패치 잔재를 삭제한다.
4. 다음 명령을 실행한다.
...
R12는 성공한 dist-budget 보고서의 top-level pass/fail 계약을 복구하고 baseline capture 직전에 보고서를 재생성한다. 런타임, 에셋, 성능 상한과 승인 baseline은 변경하지 않는다.
