...
export class MissionBoardScene extends Phaser.Scene {
  ...
  private drawHeader(): void {
    ...
    this.add.text(480, 82, '일일/주간 목표를 달성하고 보급 상자를 획득하세요', {
      fontSize: '17px', color: '#d8f3ff', fontStyle: 'bold', stroke: '#07131a', strokeThickness: 4,
    }).setOrigin(0.5).setDepth(5);
  }
  ...
  private drawMissionCard(x: number, y: number, mission: ReturnType<typeof getMissionStates>[number]): void {
    ...
    const title = this.add.text(-136, -27, mission.title, { fontSize: '18px', color: '#2b160a', fontStyle: 'bold' }).setOrigin(0, 0.5);
    const sub = this.add.text(-136, -5, mission.subtitle, { fontSize: '13px', color: '#4a2915', fontStyle: 'bold', wordWrap: { width: 210 } }).setOrigin(0, 0.5);
    const progress = this.add.text(-136, 24, `${mission.progress}/${mission.target}  ·  ${mission.reward}`, { fontSize: '13px', color: '#fff7d8', fontStyle: 'bold', stroke: '#1e0c04', strokeThickness: 3 }).setOrigin(0, 0.5);
    ...
    const btnText = this.add.text(112, 18, mission.claimed ? '완료' : mission.completed ? '수령' : '진행중', { fontSize: '13px', color: '#fff7d8', fontStyle: 'bold', stroke: '#1b0703', strokeThickness: 3 }).setOrigin(0.5);
  ...
  private drawChestPanel(): void {
    ...
    this.chestText = this.add.text(0, 22, `보급 상자 x${getRewardChestCount()}`, { fontSize: '19px', color: '#fff1bf', fontStyle: 'bold', stroke: '#2b1208', strokeThickness: 5 }).setOrigin(0.5);
    const btn = this.add.rectangle(0, 75, 144, 38, 0x8d3a25, 0.95).setStrokeStyle(2, 0xffe29a, 0.6);
    const text = this.add.text(0, 75, '상자 열기', { fontSize: '17px', color: '#fff7d8', fontStyle: 'bold', stroke: '#1b0703', strokeThickness: 3 }).setOrigin(0.5);
  ...
  private makeButton(x: number, y: number, label: string, cb: () => void, width = 180): Phaser.GameObjects.Container {
    ...
    const text = this.add.text(0, 0, label, { fontSize: '18px', color: '#fff7d8', fontStyle: 'bold', stroke: '#2a1208', strokeThickness: 4 }).setOrigin(0.5);
  ...
  private toast(text: string): void {
    const label = this.add.text(480, 456, text, { fontSize: '22px', color: '#fff1bf', fontStyle: 'bold', stroke: '#2b1208', strokeThickness: 6 }).setOrigin(0.5).setDepth(80);
