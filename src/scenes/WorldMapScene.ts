...
export class WorldMapScene extends Phaser.Scene {
  ...
  init(data: { user: User; save: PlayerSave; stageId?: StageId }): void {
    this.user = data.user;
    this.save = data.save;
    const requested = data.stageId
      ? STAGE_LIST.findIndex((stage) => stage.id === data.stageId)
      : -1;
    this.selectedIndex =
      requested >= 0 ? requested : this.findFirstPlayableIndex();
    this.selectedStage =
      STAGE_LIST[this.selectedIndex] ?? getStageConfig("stage_001");
  }
  ...
  create(): void {
    ...
    installReferenceRewardPipeline(this, { phase: "world", delayMs: 820 });
  ...
  private createIllustratedWorldMap(): void {
    const key = this.textures.exists("v1-worldmap-splash-v18")
      ? "v1-worldmap-splash-v18"
      : this.textures.exists("v1-worldmap-splash")
        ? "v1-worldmap-splash"
        : "v1-worldmap-bg";
  ...
  private createStagePreviewLayer(): void {
    this.previewImage = this.add
      .image(815, 222, this.stageCardTextureKey(this.selectedStage.id))
    ...
    const outer = this.add
      .circle(0, 0, 27, 0x7ce8ff, 0.22)
      .setBlendMode(Phaser.BlendModes.ADD);
    const ring = this.add
      .circle(0, 0, 22, 0xffffff, 0)
      .setStrokeStyle(4, 0xfff0a6, 0.94);
    const inner = this.add
      .circle(0, 0, 14, 0x2d75d6, 0.16)
  ...
  private createStageNodeHotspots(): void {
    ...
    this.addHotspot({
      ...
      radius: 12,
      tone: "gold",
      onClick: () => this.selectStage("stage_001"),
    });
    this.addHotspot({
      x: 131,
      y: 269,
      ...
      radius: 12,
      tone: "blue",
      onClick: () => this.selectStage("stage_002"),
    });
  }
  ...
  private createNavigationHotspots(): void {
    ...
    this.addHotspot({
      ...
      radius: 24,
      tone: "red",
      onClick: () => this.startSelectedStage(),
    });
    ...
    this.addHotspot({
      ...
      tone: "gold",
      onClick: () =>
        this.showToast("골드 재화 패널은 상점 패스에서 연결합니다."),
    });
  }
  ...
  private addHotspot(options: HotspotOptions): void {
    ...
    hover.lineStyle(2, 0xffffff, 0.5);
    ...
    const shine = this.add
      .rectangle(
        ...
        -options.height * 0.22,
        options.width * 0.5,
        Math.max(4, options.height * 0.15),
        0xffffff,
        0.16,
      )
      .setRotation(-0.15)
  ...
  private refreshSelectedStage(animated: boolean): void {
    ...
    const stageNode =
      STAGE_NODES.find((node) => node.id === stage.id) ?? STAGE_NODES[0];
  ...
  private isStageUnlocked(stage: StageConfig): boolean {
    return (
      ...
      Boolean(this.save.clearedStages[stage.unlockRequires]?.bestStars)
    );
  }
  ...
  private showToast(message: string, holdMs = 1500): void {
    if (
      !this.scene.isActive("WorldMapScene") ||
      !this.toastBack ||
      !this.toastText
    )
      return;
    ...
    this.toastBack.fillStyle(0x071c3e, 0.7);
    ...
    this.toastBack.lineStyle(1, 0xffffff, 0.3);
