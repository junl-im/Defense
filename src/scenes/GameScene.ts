...
import {
  castHeroStompPremiumFx,
  castMercenaryGateFx,
  castPremiumMeteor,
  openBossArenaRift,
  showArcaneSurge,
  showBossArenaPattern,
} from "../game/SpellEffects";
...
import {
  ...
  addPremiumPanelGlints,
  addTowerPanelSurface,
  installPremiumButtonFx,
  showBattleStartLoading,
  showPremiumChestCharge,
  showPremiumToast,
} from "../game/PremiumFlowUi";
...
import {
  ...
  BATTLE_SAFE_TOP,
  clampToBattlefield,
  installCombatVisualDirector,
  isBattlefieldPoint,
} from "../game/BattleVisualDirector";
import {
  addStageV26Decor,
  applyWaveEventOpening,
  mutateEnemyForWaveEvent,
  type WaveEventRuntime,
} from "../game/CampaignExpansionV26";
import {
  applyRunModifiersToEnemy,
  getRunStartAdjustments,
  getStageRunModifiers,
  getTowerAura,
  runModifierColor,
  runModifierSummary,
  type RunModifier,
} from "../game/RunModifiers";
import {
  createTacticalOrderState,
  pickTacticalOrderChoices,
  shouldOfferTacticalOrder,
  applyTacticalOrderChoice,
  renderTacticalOrderCard,
  tacticalOrderSummary,
  type TacticalOrderChoice,
  type TacticalOrderState,
} from "../game/TacticalOrdersV27";
import {
  battleContractDetailLines,
  battleContractHudLine,
  battleContractResultLines,
  createBattleContractState,
  createStageBattleContracts,
  recordBattleContractEvent,
  type BattleContractEvent,
  type BattleContractState,
} from "../game/BattleContractsV28";
import {
  applyEnemyAffixV29,
  enemyAffixFullLineV29,
  enemyAffixHudLineV29,
  NO_ENEMY_AFFIX_V29,
  pickEnemyAffixV29,
  type EnemyAffixRuntimeV29,
} from "../game/EnemyAffixesV29";
import {
  capCombatDeltaV29,
  isLiteModeV29,
  mobileShortNumberV29,
  recommendTowerForWaveV29,
  towerRoleLineV29,
} from "../game/MobileBattleAdvisorV29";
import {
  V210_BUILD_HIT,
  V210_BUILD_MENU,
  V210_TOWER_HIT,
...
export class GameScene extends Phaser.Scene {
  ...
  private battleContractState: BattleContractState =
    createBattleContractState();
  ...
  init(data: {
    ...
    this.gold =
      this.stage.startGold +
      this.relicBonuses.startGoldBonus +
      heroBonus.startGold +
      runStart.gold;
    if (this.dailyChallenge?.modifiers.includes("gold_rush"))
      this.gold = Math.round(this.gold * 1.25);
    ...
    this.battleContractState = createStageBattleContracts(
      this.stage,
      data.user?.uid ?? "guest",
    );
  ...
  create(): void {
    ...
    showBattleStartLoading(
      this,
      this.stage.title,
      "전술 배치 · 공세 분석 · 지휘 HUD 전개",
    );
    this.showRunModifierBanner();
    safeDelayedCall(this, 760, () => this.showTacticalOrderDraft("opening"), {
      canRun: () => this.isSceneLive(),
    });
    safeDelayedCall(this, 920, () => this.refreshV29AdvisorForNextWave(), {
      canRun: () => this.isSceneLive(),
    });
    ...
    this.hero = new Hero(
      this,
      this.stage.path[0].x + 120,
      this.stage.path[0].y - 35,
    );
    this.hero.damage = Math.round(
      this.hero.damage *
        getHeroBattleBonus(selectedHero.id).heroDamage *
        this.tacticalOrderState.heroDamageMultiplier,
    );
    if (this.dailyChallenge?.modifiers.includes("hero_trial"))
      this.hero.damage = Math.round(this.hero.damage * 1.35);
    this.hero.on("pointerdown", () =>
      this.showMessage("영웅 레온 선택됨. 빈 맵 터치로 이동합니다."),
    );
    this.createSpells();
    ...
    if (this.dailyChallenge) {
      safeDelayedCall(
        ...
        () =>
          this.showMessage(
            `일일 도전: ${this.dailyChallenge!.modifiers.map(modifierLabel).join(" / ")}`,
          ),
        { canRun: () => this.isSceneLive() },
      );
    }
    ...
    const bossPatternHandler = (payload: {
      label: string;
      pattern: string;
      kind?: EnemyKind;
      x?: number;
    ...
    const removeBossPatternHandler = (): void => {
      this.events.off("kingdom-seed:boss-pattern", bossPatternHandler);
    };
  ...
  update(_: number, delta: number): void {
    ...
    this.mercenaryCooldownMs = Math.max(
      0,
      this.mercenaryCooldownMs - safeDelta,
    );
    this.tacticalHintCooldownMs = Math.max(
      0,
      this.tacticalHintCooldownMs - safeDelta,
    );
    if (
      !this.waveRunning &&
      this.waveAutoTimer &&
      ...
      this.nextWaveCountdownMs = Math.max(
        0,
    ...
    this.mercenaries.forEach((soldier) =>
      soldier.update(safeDelta, this.enemies),
    );
    ...
    if (
      ...
      if (this.waveIndex >= this.stage.waves.length - 1)
        ...
        safeDelayedCall(this, 260, () => this.showTacticalOrderDraft("wave"), {
          canRun: () => this.isSceneLive(),
        });
      }
    }
  ...
  private drawHudChrome(): void {
    if (this.textures.exists("v1-combat-top-hud")) {
      this.add
        .image(480, 24, "v1-combat-top-hud")
        .setDisplaySize(930, 44)
        .setDepth(70);
    } else if (this.textures.exists("ui-hud-top-panel")) {
      this.add
        .image(480, 32, "ui-hud-top-panel")
        .setDisplaySize(960, 64)
        .setDepth(70);
    } else {
    ...
    if (this.textures.exists("v1-combat-bottom-dock")) {
      this.add
        .image(480, 511, "v1-combat-bottom-dock")
        .setDisplaySize(916, 54)
        .setDepth(70);
    } else if (this.textures.exists("ui-hud-bottom-panel")) {
      this.add
        .image(480, 510, "ui-hud-bottom-panel")
        .setDisplaySize(960, 64)
        .setDepth(70);
    } else {
  ...
  private createAmbientMapFx(): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const mote = this.add
        .circle(x, y, theme === "swamp" ? 5 : 3, color, alpha)
        .setDepth(5);
      this.tweens.add({
        targets: mote,
  ...
  private drawCanyonDetails(): void {
    for (let i = 0; i < 20; i++) {
      ...
      if (this.nearPath(x, y, 58)) continue;
      this.add
        .polygon(x, y, [0, -22, 24, 18, -18, 16], 0x8f5633, 1)
        .setStrokeStyle(2, 0x3f2017, 0.35)
        .setDepth(2);
      if (i % 4 === 0)
        this.add.circle(x + 20, y - 12, 6, 0xffd06b, 0.32).setDepth(3);
    }
  ...
  private drawSwampDetails(): void {
    for (let i = 0; i < 26; i++) {
      ...
      if (i % 5 === 0)
        this.add.circle(x + 16, y - 14, 5, 0xc2ff9a, 0.2).setDepth(4);
    }
  ...
  private drawFortressDetails(): void {
    ...
    for (let i = 0; i < 13; i++) {
      const x = 28 + i * 76;
      this.add
        .rectangle(x, 92, 44, 70, 0x3a3036, 1)
        .setStrokeStyle(2, 0x0e0a0d, 0.35)
        .setDepth(2);
      this.add
        .triangle(x, 42, -25, 28, 0, -14, 25, 28, 0x6c1f2a, 1)
        .setDepth(3);
      if (i % 2 === 0)
        this.add.circle(x + 18, 122, 5, 0xff7a42, 0.36).setDepth(4);
    }
    for (let i = 0; i < 18; i++) {
      const x = (i * 73 + 55) % 930;
      ...
      this.add
        .rectangle(x, y, 54, 22, 0x3d342c, 0.95)
        .setRotation(((i % 5) - 2) * 0.08)
        .setStrokeStyle(1, 0x0e0a0d, 0.35)
        .setDepth(2);
      if (i % 3 === 0)
        this.add.circle(x + 24, y - 10, 7, 0xffb347, 0.18).setDepth(3);
    }
    this.add
      .circle(850, 112, 44, 0xff5a3d, 0.12)
      .setStrokeStyle(2, 0xffb347, 0.2)
      .setDepth(3);
  }
  ...
  private createRunModifierHud(): void {
    ...
    this.tweens.add({
      ...
      yoyo: true,
      hold: 1700,
      onComplete: () => this.runModifierText?.setAlpha(0),
    });
  ...
  private refreshArmySynergy(): void {
    ...
    if (this.tacticalOrderState.chosen.length > 0 && label === "기본")
      label = "작전 지휘";
    this.commandAuraLabel = label;
    this.towers.forEach((tower) =>
      tower.setCommandAura(damageMultiplier, fireRateMultiplier, label),
    );
    const damageText = `${Math.round((damageMultiplier - 1) * 100)}%`;
    const speedText =
      fireRateMultiplier < 1
        ? ` / 속도 +${Math.round((1 - fireRateMultiplier) * 100)}%`
        : "";
    this.synergyText.setText(
      kinds.size > 0
        ? `시너지 ${label} · 화력 +${damageText}${speedText}`
        : "시너지: 타워 조합 대기",
    );
  }

  private createV29CombatAdvisorHud(): void {
    const bgKey = this.textures.exists("v2-affix-chip-v29")
      ? "v2-affix-chip-v29"
      : undefined;
    if (bgKey)
      this.add
  ...
  private refreshV29AffixHud(): void {
    ...
    if (this.enemyAffixText)
      this.enemyAffixText.setColor(
        this.activeEnemyAffix.id === "none" ? "#eaf6ff" : "#fff4c2",
      );
  }
  ...
  private refreshV29AdvisorForNextWave(): void {
    ...
    const nextIndex = this.waveRunning
      ? this.waveIndex
  ...
  private refreshTacticalOrderHud(): void {
    this.tacticalOrderHudText?.setText(
      tacticalOrderSummary(this.tacticalOrderState),
    );
  }
  ...
  private refreshBattleContractHud(): void {
    this.battleContractHudText?.setText(
      battleContractHudLine(this.battleContractState),
    );
    this.battleContractDetailText?.setText(
      battleContractDetailLines(this.battleContractState).join("   "),
    );
  }
  ...
  private showBattleContractSummary(): void {
    const lines = this.battleContractState.contracts
      .map(
        (contract) =>
          `${contract.completed ? "완료" : contract.failed ? "실패" : "진행"} · ${contract.title}  ${contract.progress}/${contract.goal}`,
      )
      .join(" / ");
  ...
  private recordBattleContract(event: BattleContractEvent): void {
    const completed = recordBattleContractEvent(
      this.battleContractState,
      event,
    );
    completed.forEach((contract, index) => {
      ...
      safeDelayedCall(
        ...
        () => {
          if (!this.ended)
            this.showMessage(
              `전장 계약 완료: ${contract.shortTitle} +$${contract.rewardGold}`,
            );
        },
        { canRun: () => this.isSceneLive() },
      );
  ...
  private showTacticalOrderDraft(reason: "opening" | "wave" | "manual"): void {
    ...
    const choices = pickTacticalOrderChoices(
      this.stage,
      this.user?.uid ?? "guest",
      this.waveIndex,
      this.tacticalOrderState,
    ...
    const blocker = this.add
      .rectangle(0, 0, 960, 540, 0x020611, 0.48)
      .setInteractive({ useHandCursor: false });
    const panel = this.textures.exists("v2-order-panel-v27")
      ? this.add.image(0, 4, "v2-order-panel-v27").setDisplaySize(820, 286)
      : this.add
          .rectangle(0, 4, 820, 286, 0x07101e, 0.92)
          .setStrokeStyle(3, 0xffd56c, 0.62);
    const title = this.add
      .text(
        0,
        -122,
        reason === "opening" ? "오늘의 작전 명령 선택" : "전황 카드 선택",
        {
          fontFamily: "Pretendard, Noto Sans KR, Arial, sans-serif",
          fontSize: "18px",
          fontStyle: "bold",
    ...
    choices.forEach((choice, index) => {
      const card = renderTacticalOrderCard(
        this,
        -248 + index * 248,
        18,
        choice,
        () => this.pickTacticalOrder(choice),
      );
      root.add(card);
    });
  ...
  private pickTacticalOrder(choice: TacticalOrderChoice): void {
    ...
    if (effects.instantGold)
      this.gold += Math.round(effects.instantGold + this.stage.number * 6);
    if (effects.healLives) this.lives += effects.healLives;
    if (effects.cooldownReductionMs)
      this.reduceAllCooldowns(effects.cooldownReductionMs);
    if (effects.heroDamageMultiplier && this.hero)
      this.hero.damage = Math.round(
        this.hero.damage * effects.heroDamageMultiplier,
      );
    if (effects.overdriveMs)
      this.towers.forEach((tower) =>
        tower.activateOverdrive(effects.overdriveMs),
      );
    this.refreshArmySynergy();
    this.forceHudRefresh();
  ...
  private reduceAllCooldowns(amountMs: number): void {
    ...
    if (this.hero)
      this.hero.skillCooldownMs = Math.max(
        0,
        this.hero.skillCooldownMs - amountMs,
      );
  }
  ...
  private getSafeMapPoint(x: number, y: number): { x: number; y: number } {
    ...
    return {
      x: Phaser.Math.Clamp(
        x,
        Math.max(88, BATTLE_SAFE_LEFT + 56),
        Math.min(872, BATTLE_SAFE_RIGHT - 56),
      ),
      y: Phaser.Math.Clamp(y, BATTLE_SAFE_TOP + 30, BATTLE_SAFE_BOTTOM - 28),
    };
  }
  ...
  private clampOverlayPosition(
    ...
    const topReserve = BATTLE_SAFE_TOP - 4;
  ...
  private installSceneCleanup(): void {
    const cleanup = (): void => {
      this.waveAutoTimer = clearTimer(this.waveAutoTimer);
      this.waveAutoTimer = undefined;
      this.spawnEvents.forEach((event) => event.remove(false));
      this.spawnEvents = [];
      ...
      this.messageHideTimer = undefined;
      this.comboHideTimer = clearTimer(this.comboHideTimer);
      this.comboHideTimer = undefined;
  ...
  private createUiInputGuards(): void {
    ...
    makeGuard(
      480,
      (540 + BATTLE_SAFE_BOTTOM) / 2,
      960,
      540 - BATTLE_SAFE_BOTTOM,
      "skill dock safe zone",
    );
  }

  private beginSpellTargeting(
    kind: Exclude<CastingSpell, undefined>,
    label: string,
    radius: number,
    color: number,
  ): void {
    ...
    const base = this.textures.exists("v1-target-reticle")
      ? this.add
          .image(0, 0, "v1-target-reticle")
          .setDisplaySize(radius * 2.3, radius * 2.3)
          .setAlpha(0.82)
          .setBlendMode(Phaser.BlendModes.ADD)
      : this.add
          .circle(0, 0, radius, color, 0.08)
          .setStrokeStyle(2, color, 0.62);
    const fill = this.add
      .circle(0, 0, radius, color, kind === "meteor" ? 0.06 : 0.035)
      .setStrokeStyle(2, color, 0.48)
      .setBlendMode(Phaser.BlendModes.ADD);
    const dot = this.add
      .circle(0, 0, 5, 0xffffff, 0.78)
  ...
  private updateSpellTargetPreview(pointer: Phaser.Input.Pointer): void {
    ...
    const target = safePoint
      ...
      : clampToBattlefield(pointer.x, pointer.y);
    this.spellTargetPreview.setPosition(target.x, target.y);
    this.spellTargetPreview.setAlpha(safePoint ? 1 : 0.5);
    this.spellTargetLabel?.setText(
      safePoint
  ...
  private createBuildSpot(x: number, y: number, autoOpen = false): void {
    ...
    const light = this.add
      .ellipse(x - 5, y - 3, 12, 4, 0xffe1a0, 0.22)
      .setDepth(14);
    ...
    const tagBg = this.add
      .rectangle(x, y + 18, 42, 13, 0x130d09, 0.78)
    ...
    if (premiumPad) {
      ...
      tagBg
        .setAlpha(0.58)
        .setFillStyle(0x07101e, 0.58)
        .setStrokeStyle(1, 0x8fdcff, 0.42);
    ...
    const largeHitZone = this.add
      .rectangle(
        x,
        y + 1,
        V210_BUILD_HIT.width,
        V210_BUILD_HIT.height,
        0xffffff,
        0.001,
      )
      .setDepth(19)
  ...
  private openBuildMenu(
    ...
    this.destroySelectedPanel();
    this.destroyBuildMenu();
    const menuPos = this.clampOverlayPosition(
      x,
      y - 8,
      V210_BUILD_MENU.width,
      V210_BUILD_MENU.height,
      7,
      80,
    );
    ...
    if (Math.abs(menuPos.x - x) > 4 || Math.abs(menuPos.y - y) > 4) {
      const guide = this.add
        .line(0, 0, x - menuPos.x, y - menuPos.y, 0, -76, 0xffd36b, 0.42)
        .setLineWidth(2)
        .setAlpha(0.72);
      menu.add(guide);
    }
    const bg = this.textures.exists("v1-tower-build-menu")
      ? this.add
          .image(0, 0, "v1-tower-build-menu")
          .setDisplaySize(V210_BUILD_MENU.width, V210_BUILD_MENU.height)
      : this.textures.exists("ui-build-menu-frame-v47")
        ? this.add
            .image(0, 0, "ui-build-menu-frame-v47")
            .setDisplaySize(V210_BUILD_MENU.width, V210_BUILD_MENU.height)
        : this.add
            .rectangle(
              0,
    ...
    const nextGroups =
      this.stage.waves[
        Math.max(0, this.waveRunning ? this.waveIndex : this.waveIndex + 1)
      ] ?? [];
    ...
    const positions = [
      { kind: "archer" as TowerKind, x: -64, y: -10 },
      { kind: "mage" as TowerKind, x: 64, y: -10 },
      { kind: "barracks" as TowerKind, x: -64, y: 34 },
      { kind: "artillery" as TowerKind, x: 64, y: 34 },
    ];
    ...
    positions.forEach(({ kind, x: bx, y: by }) => {
      ...
      const card = this.textures.exists("v1-tower-build-card")
        ? this.add
            .image(bx, by, "v1-tower-build-card")
            .setDisplaySize(
              V210_BUILD_MENU.cardWidth,
              V210_BUILD_MENU.cardHeight,
            )
            .setInteractive({ useHandCursor: true })
        : this.add
            .rectangle(
              ...
              V210_BUILD_MENU.cardWidth,
              V210_BUILD_MENU.cardHeight,
              canBuy ? 0x23170f : 0x2b2b2b,
              0.98,
            )
            .setStrokeStyle(
              2,
              canBuy ? cfg.color : 0x5b5b5b,
              canBuy ? 0.65 : 0.45,
            )
            .setInteractive({ useHandCursor: true });
      ...
      card.on("pointerdown", () => {
        ...
        const towerClickRing = this.textures.exists("v2-tower-selection-ring")
          ? this.add
              .image(x, y + 1, "v2-tower-selection-ring")
              .setDisplaySize(50, 32)
              .setDepth(21)
              .setAlpha(0)
              .setBlendMode(Phaser.BlendModes.ADD)
          : this.textures.exists("ui-tower-click-ring-v47")
            ? this.add
                .image(x, y + 2, "ui-tower-click-ring-v47")
                .setDisplaySize(148, 128)
                .setDepth(21)
                .setAlpha(0)
            : undefined;
        const towerHitHalo = this.add
          .zone(
            x,
            y + V210_TOWER_HIT.offsetY,
            V210_TOWER_HIT.width,
            V210_TOWER_HIT.height,
          )
          .setDepth(76)
          .setInteractive({ useHandCursor: true });
  ...
  private towerCost(kind: TowerKind, baseCost: number): number {
    let cost = baseCost;
    if (this.dailyChallenge?.modifiers.includes("no_mage") && kind === "mage")
      cost = Math.round(cost * 1.65);
    if (this.dailyChallenge?.modifiers.includes("iron_wall") && kind === "mage")
      cost = Math.round(cost * 0.92);
    cost = Math.max(
      1,
      Math.round(cost * this.tacticalOrderState.costMultiplier),
    );
    return cost;
  }
  ...
  private createTowerPanel(tower: Tower): void {
    ...
    const panelHeight = hasMasteryChoices
      ? 228
      : tower.config.kind === "barracks"
        ? 194
        : 180;
    const panelWidth = V210_TOWER_PANEL.width;
    const preferredX =
      tower.x < 500
        ? tower.x + V210_TOWER_PANEL.compactOffset
        : tower.x - V210_TOWER_PANEL.compactOffset;
    const preferredY = tower.y + 12;
    const panelPos = this.clampOverlayPosition(
      preferredX,
      preferredY - 6,
      panelWidth,
      panelHeight + 18,
    ...
    if (
      Math.abs(panelPos.x - preferredX) > 8 ||
      Math.abs(panelPos.y - preferredY) > 8
    ) {
    ...
    if (this.textures.exists("v1-tower-command-panel")) {
      panel.add(
        this.add
          .image(0, 0, "v1-tower-command-panel")
          .setDisplaySize(panelWidth, panelHeight + 12),
      );
    } else {
      addTowerPanelSurface(
        this,
    ...
    const icon = this.add
      .circle(-110, -panelHeight / 2 + 24, 13, tower.config.color, 0.9)
    ...
    const masteryLabel =
      tower.level >= 3
        ? `전문화: ${tower.masteryLabel}`
        : `Lv.3 특수: ${tower.config.maxSkill}`;
    const masteryLine = this.add.text(
      -114,
      -panelHeight / 2 + 52,
      masteryLabel,
      {
    ...
    const statBox = this.add
      .rectangle(0, -panelHeight / 2 + 88, 218, 26, 0x050914, 0.58)
    ...
    if (hasMasteryChoices) {
      ...
      choices.forEach((choice, index) => {
        ...
        const btn = this.makePanelButton(
          panel,
          x,
          y,
          98,
          22,
          canBuy ? choice.color : 0x333333,
          `${choice.shortLabel} $${tower.masteryCost}`,
        );
    ...
    const up = this.makePanelButton(
      panel,
      -55,
      actionY,
      98,
      22,
      cost ? 0x24486b : 0x333333,
      cost ? `업그레이드 $${cost}` : "최고 레벨",
    );
    up.on("pointerdown", () => this.upgradeSelectedTower());

    const overdriveCost = this.towerOverdriveCost(tower);
    const overLabel = tower.isOverdriven
      ? `강화중 ${tower.overdriveRemainingSec}s`
      : `긴급 강화 $${overdriveCost}`;
    const over = this.makePanelButton(
      panel,
      55,
      actionY,
      98,
      22,
      tower.isOverdriven ? 0x3a3a3a : 0x6a4a1f,
      overLabel,
    );
    over.on("pointerdown", () => this.overdriveSelectedTower());

    if (tower.config.kind !== "barracks") {
      const target = this.makePanelButton(
        panel,
        -55,
        actionY + 27,
        98,
      ...
      const rally = this.makePanelButton(
        panel,
        -55,
        actionY + 27,
        98,
      ...
      const reinforce = this.makePanelButton(
        panel,
        55,
        actionY + 27,
        98,
    ...
    const sell = this.makePanelButton(
      panel,
      -55,
      actionY + 54,
      98,
    ...
    const replace = this.makePanelButton(
      panel,
      55,
      actionY + 54,
      98,
    ...
    const close = this.makePanelButton(
      panel,
      0,
      panelHeight / 2 - 13,
      80,
  ...
  private chooseTowerMastery(id: TowerMasteryId): void {
    ...
    if (tower.chooseMastery(id)) {
      ...
      this.showMessage(
        `${tower.masteryLabel} 완성! 전술 성능이 크게 변화합니다.`,
      );
    }
  }
  ...
  private overdriveSelectedTower(): void {
    ...
    if (tower.isOverdriven) {
      this.showMessage(
        `이미 긴급 강화 중입니다 (${tower.overdriveRemainingSec}s)`,
      );
      return;
    }
  ...
  private towerStatLine(tower: Tower): string {
    ...
    const splash = tower.currentSplashRadius
      ? ` · 폭발 ${Math.round(tower.currentSplashRadius)}`
      : "";
  ...
  private createSpells(): void {
    ...
    meteor.on("pointerdown", () => {
      if (this.meteorCooldownMs > 0)
        return this.showMessage("메테오 쿨타임 중");
    ...
    mercenary.on("pointerdown", () => {
      if (this.mercenaryCooldownMs > 0)
        return this.showMessage("용병소환 쿨타임 중");
      pulseButton(this, mercenary);
      playSfx(this, "sfx_click");
      this.beginSpellTargeting(
        "mercenary",
        "용병 소환 지점 선택",
        36,
        0x7cc7ff,
  ...
  private createInputHandlers(): void {
    this.input.on("pointermove", (p: Phaser.Input.Pointer) =>
      this.updateSpellTargetPreview(p),
    );
    this.input.on(
      "pointerdown",
      (
        ...
        if (!isBattlefieldPoint(p.x, p.y)) {
          if (this.castingSpell)
            this.showMessage("스킬은 전장 안쪽에만 사용할 수 있습니다.");
          return;
        }
  ...
  private castMeteor(x: number, y: number): void {
    ...
    this.meteorCooldownMs = Math.round(
      24000 *
        this.relicBonuses.meteorCooldownMultiplier *
        (this.dailyChallenge?.modifiers.includes("meteor_storm") ? 0.72 : 1),
    );
    castPremiumMeteor(this, x, y, radius, () => {
      ...
      const meteorDamage = Math.round(
        112 * this.tacticalOrderState.meteorDamageMultiplier,
      );
      this.enemies.forEach((enemy) => {
        if (
          ...
          enemy.receiveDamage(meteorDamage, "true");
      });
    });
  }
  ...
  private refreshWavePreview(): void {
    ...
    if (this.waveRunning) {
      ...
      renderWaveIntelPanel(this, this.waveIntelPanel, current, {
        inCombat: true,
      });
      return;
    }
    ...
    renderWaveIntelPanel(this, this.waveIntelPanel, next, {
      nextWaveNumber: nextIndex + 1,
    });
    this.refreshV29AdvisorForNextWave();
  }
  ...
  private refreshSpellHud(): void {
    this.meteorText.setText(
      this.meteorCooldownMs > 0
        ? `☄ ${Math.ceil(this.meteorCooldownMs / 1000)}s`
        : "☄ 메테오",
    );
    this.mercenaryText.setText(
      this.mercenaryCooldownMs > 0
        ? `🛡️ ${Math.ceil(this.mercenaryCooldownMs / 1000)}s`
        : "🛡️ 용병소환",
    );
    this.heroSkillText.setText(
      this.hero.skillCooldownMs > 0
        ? `🦁 ${Math.ceil(this.hero.skillCooldownMs / 1000)}s`
        : "🦁 대지강타",
    );
  }
...
속도 ${this.gameSpeed}x / Wave ${Math.max(0, this.waveIndex + 1)}/${this.stage.waves.length}`,
  ...
  private showBossWarning(): void {
    const warning = this.add
      .text(480, 118, "⚠ 보스 웨이브 접근 ⚠", {
        fontSize: "32px",
        color: "#ffb347",
        fontStyle: "bold",
        backgroundColor: "#2b0f0faa",
        padding: { x: 18, y: 8 },
      })
      .setOrigin(0.5)
      .setDepth(95);
  ...
  private scheduleNextWave(delayMs: number, initial = false): void {
    ...
    if (initial)
      this.showMessage(
        "방어 성공! 다음 공격까지 10초. 준비됐다면 [진행]을 누르세요.",
      );
    this.waveAutoTimer = safeDelayedCall(
      this,
      delayMs,
  ...
  private startNextWave(early: boolean): void {
    ...
    this.activeEnemyAffix = pickEnemyAffixV29(
      this.stage,
      this.user?.uid ?? "guest",
      this.waveIndex,
    );
    ...
    const bossGroup = waveGroups.find(
      (group) => ENEMIES[group.kind].threat === "boss",
    );
    if (bossGroup) {
    ...
    if (this.activeEnemyAffix.id !== "none")
      safeDelayedCall(
        this,
        380,
        () => this.showMessage(enemyAffixFullLineV29(this.activeEnemyAffix)),
        { canRun: () => this.isSceneLive() },
      );
  ...
  private spawnWave(groups: WaveSpawn[]): void {
    ...
    groups.forEach((group) => {
      const count =
        group.count +
        (this.dailyChallenge?.modifiers.includes("gold_rush")
          ? Math.max(1, Math.floor(group.count * 0.16))
          : 0);
      ...
      const spawnOne = (): void => {
        ...
        if (this.ended) return;
        globalSpawnIndex += 1;
        let cfg = mutateEnemyForWaveEvent(
          { ...ENEMIES[group.kind] },
          this.activeWaveEvent,
        );
        cfg = applyRunModifiersToEnemy(
          cfg,
          this.runModifiers,
          this.waveIndex + 1,
          globalSpawnIndex,
        ...
        if (
          this.dailyChallenge?.modifiers.includes("iron_wall") &&
          (cfg.threat === "tank" || cfg.threat === "boss")
        ) {
          cfg.armor = Math.min(0.78, cfg.armor + 0.12);
          cfg.hp = Math.round(cfg.hp * 1.08);
        }
        if (
          this.dailyChallenge?.modifiers.includes("boss_contract") &&
          cfg.threat === "boss"
        ) {
        ...
        if (
          ...
          const badge = this.add
            .image(enemy.x, enemy.y - 44, "v2-elite-badge")
        ...
        if (cfg.threat === "boss" || cfg.magicResist > 0.3)
          showArcaneSurge(
            this,
            enemy.x,
            enemy.y,
            enemy.config.accentColor ?? 0x8fdcff,
          );
      };

      const opener = safeDelayedCall(this, delay, () => {
        if (!this.isSceneLive()) return;
        spawnOne();
        if (count > 1) {
          const stream = this.time.addEvent({
            delay: gap,
            repeat: count - 2,
            callback: () => {
              if (this.isSceneLive()) spawnOne();
            },
          });
  ...
  private describeWave(groups: WaveSpawn[]): string {
    return groups
      .map((group) => `${ENEMIES[group.kind].label} x${group.count}`)
      .join("  /  ");
  }
  ...
  private registerKill(enemy: Enemy): void {
    ...
    const threatBonus =
      enemy.config.threat === "boss"
        ? 8
        : enemy.config.threat === "tank"
          ? 3
          : 1;
    const streakBonus =
      this.killStreak >= 10
        ? Math.min(14, Math.floor(this.killStreak / 5) * 2)
        : 0;
    const reward = enemy.config.reward + streakBonus;
    this.gold += reward;
    this.score +=
      enemy.config.reward * 10 * threatBonus +
      this.lives * 2 +
      this.killStreak * 3;
    ...
    if (enemy.config.threat === "boss")
      this.showTacticalHint(
        "보스 격파! 남은 웨이브는 타워 교체/진화로 마무리하세요.",
      );
  }
  ...
  private showComboToast(streak: number, goldBonus: number): void {
    ...
    this.comboHideTimer = clearTimer(this.comboHideTimer);
    this.comboText
      .setText(`x${streak} 연속 처치${goldBonus > 0 ? `  +$${goldBonus}` : ""}`)
      .setVisible(true)
      .setAlpha(1)
      .setScale(1);
    ...
    this.comboHideTimer = safeDelayedCall(
      this,
      980,
      () => {
  ...
  private refreshObjectivePanel(): void {
    ...
    this.objectiveText.setText(
      `${lifeMark} 생명 ${lifeGoal}+   ${leakMark} 무누수   ${chainMark} 연속처치 20`,
    );
  }
    ...
    safeDelayedCall(
      ...
      () => {
        ...
        this.tweens.add({
          ...
          alpha: 0,
          duration: 240,
          onComplete: () => this.directorText?.setVisible(false),
        });
      },
      { canRun: () => this.isSceneLive() },
    );
  ...
  private showWaveTacticalHint(groups: WaveSpawn[]): void {
    const advisor = recommendTowerForWaveV29(groups, ENEMIES);
    if (this.waveIndex <= 1)
      this.showTacticalHint(`${advisor.title}: ${advisor.reason}`);
    ...
    const hasMagic = groups.some(
      (group) => ENEMIES[group.kind].magicResist > 0.35,
    );
    const hasBoss = groups.some(
      (group) => ENEMIES[group.kind].threat === "boss",
    );
    if (hasBoss)
      this.showTacticalHint(
        "보스 공세: 긴급 강화와 최종 진화 스킬을 아끼지 마세요.",
      );
    else if (hasFlying)
      this.showTacticalHint(
        "공중 적 감지: 궁수/마법 타워를 선두 라인에 배치하세요.",
      );
    else if (hasArmor)
      this.showTacticalHint("장갑 적 감지: 마법사와 저주 계열이 효율적입니다.");
    else if (hasMagic)
      this.showTacticalHint("마법 저항 적 감지: 포탑과 궁수 화력을 겹치세요.");
    else
      this.showTacticalHint("균형 공세: 병영으로 묶고 광역 타워로 정리하세요.");
  }
  ...
  private async finishStage(): Promise<void> {
    ...
    const finalScore =
      this.score +
      this.battleContractState.contractScoreEarned +
      this.lives * 500 +
      Math.max(0, 600000 - clearTimeMs) / 100 +
  ...
  private showResult(
    score: number,
    top: Array<{ nickname: string; score: number }>,
    clearTimeMs: number,
  ): void {
    ...
    const openChest = this.makeUiButton(
      250,
      464,
      172,
      48,
    ...
    openChest.on("pointerdown", () => {
      ...
      safeDelayedCall(
        ...
        { canRun: () => this.scene.isActive("GameScene") },
      );
    });

    const forge = this.makeUiButton(
      452,
      464,
      172,
      48,
  ...
  private buttonAssetForColor(color: number): string | undefined {
    const choose = (preferred: string, fallback: string): string =>
      this.textures.exists(preferred) ? preferred : fallback;
    if (color === 0xa94732 || color === 0x4f1f1f || color === 0x693434)
      return choose("v1-combat-button-red-v18", "v1-button-red");
    if (
      color === 0x6a4a1f ||
      color === 0x4f3d1f ||
      color === 0x754928 ||
      ...
      return choose("v1-combat-button-gold-v18", "v1-button-gold");
    if (
      color === 0x2f4f35 ||
      color === 0x3f5f2f ||
      color === 0x2f5f58 ||
  ...
  private makeUiButton(
    ...
    const shadow = this.add
      ...
      .setDepth(depth - 1);
    const image =
      assetKey && this.textures.exists(assetKey)
        ? this.add
            .image(x, y, assetKey)
  ...
  private makePanelButton(
    ...
    rect.on("pointerover", () => {
      rect.setAlpha(0.92);
      image?.setScale(1.015);
      shine.setAlpha(image ? 0.12 : 0.2);
    });
    rect.on("pointerout", () => {
      rect.setAlpha(1);
      image?.setScale(1);
      shine.setAlpha(image ? 0 : 0.12);
    });
