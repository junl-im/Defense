...
import { toggleDefenseUiFocusMode } from "./DefenseUiFocusSystem";
...
function installAdaptivePanel(scene: Phaser.Scene, profile: AdaptiveFallbackProfile): void {
  ...
  bg.fillStyle(0x020611, 0.94).fillRoundedRect(0, 0, 220, 306, 18);
  bg.lineStyle(2, 0xffdf8a, 0.72).strokeRoundedRect(0, 0, 220, 306, 18);
  ...
  const status = scene.add
    .text(110, 288, statusText(profile), {
      fontFamily: "Pretendard, Noto Sans KR, Arial, sans-serif",
      fontSize: "12px",
      fontStyle: "800",
      color: "#a8cfff",
  ...
  addPanelButton(scene, panel, 22, 182, 82, 32, "디자인", () => {
    toggleSupremeDesignGrade();
    refreshPanel(scene);
  });
  addPanelButton(scene, panel, 116, 182, 82, 32, "정리", () => {
    toggleDefenseUiFocusMode();
    refreshPanel(scene);
  });
  addPanelButton(scene, panel, 22, 224, 82, 30, "비상", () => {
    enableEmergencyFallback("panel");
    refreshPanel(scene);
  }, true);
  addPanelButton(scene, panel, 116, 224, 82, 30, "기본", () => {
