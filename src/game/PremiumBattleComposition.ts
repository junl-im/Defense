...
export function installPremiumBattleComposition(scene: Phaser.Scene, stage: StageConfig): void {
  ...
  for (const spot of stage.spots) {
    ...
    scene.add
      .ellipse(spot.x, spot.y + 8, 86, 32, 0x000000, low ? 0.13 : 0.17)
      .setDepth(9.3);
    scene.add
      .ellipse(spot.x, spot.y + 1, 78, 28, accent, low ? 0.035 : 0.052)
      .setStrokeStyle(1, accent, low ? 0.10 : 0.16)
      .setDepth(9.4);
  }
}
...
function addStaticGate(
  ...
  scene.add.ellipse(x, y + 14, 84, 24, 0x000000, low ? 0.16 : 0.22).setDepth(7.1);
  scene.add
    .ellipse(x, y, low ? 58 : 68, low ? 32 : 38, color, low ? 0.06 : 0.075)
    .setStrokeStyle(low ? 2 : 3, color, low ? 0.30 : 0.38)
    .setDepth(7.2)
    .setBlendMode(Phaser.BlendModes.ADD);
  scene.add
    .text(x, y + 31, label, {
