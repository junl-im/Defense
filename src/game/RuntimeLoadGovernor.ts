...
export function markSceneTransition(reason = "scene-transition", pauseMs = 1800): void {
...
export function optionalRuntimeWorkAllowed(kind: OptionalWorkKind, options: { scene?: Phaser.Scene; allowDuringBoot?: boolean } = {}): boolean {
  ...
  if (kind === "audio") {
    ...
    return !networkBusy() || now - state.bootStartedAt > caps.bootQuietMs + 2600;
  }
  if (kind === "pwa" || kind === "firebase") {
    return !networkBusy() && now - state.bootStartedAt > caps.bootQuietMs;
  }
  return false;
}
...
export function progressiveAssetBudgetFor(kind: "login" | "lobby" | "world" | "battle", requested: number): number {
  ...
  const absoluteSessionCap = unsafeOverride() ? 48 : explicitArtPreview() ? 12 : 0;
...
export function installRuntimeLoadGovernor(): void {
  ...
  window.addEventListener("kingdom-seed:memory-pressure", () => pauseFromEvent("memory-pressure", 9000));
  window.addEventListener("kingdom-seed:runtime-lockdown", () => pauseFromEvent("runtime-lockdown", 20000));
  window.addEventListener("pagehide", () => pauseFromEvent("pagehide", 8000));
  window.addEventListener("blur", () => pauseFromEvent("window-blur", 3600));
  window.addEventListener("offline", () => pauseFromEvent("offline", 20000));
  window.addEventListener("online", () => pauseFromEvent("online-settle", 2800));
  document.addEventListener("visibilitychange", () => {
    if (document.hidden) pauseFromEvent("hidden", 12000);
    else pauseFromEvent("visible-settle", 2200);
  });
