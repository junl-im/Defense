...
import { COMBAT_ART_TEXTURE_IDS, GUARDIAN_CITADEL_TEXTURE_ID } from '../engine/asset-catalog.js';

const DIRECTIONS = 11;
const STATES = Object.freeze(['idle', 'move', 'attack', 'skill', 'hit', 'death']);
...
function createHealthBar({ width = 1.55, height = .13, renderOrder = 40 } = {}) {
  ...
  root.name = 'worldHealthBarV110';
  root.userData.worldHealthBarV110 = true;

  const shadow = makeBarSprite(0x05070d, renderOrder);
  shadow.scale.set(width + .15, height + .10, 1);
  ...
  fill.center.set(0, .5);
  fill.scale.set(width, height * .72, 1);
  fill.position.set(-width * .5, 0, .004);

  const shine = makeBarSprite(0xffffff, renderOrder + 3);
  shine.center.set(0, .5);
  shine.scale.set(width, height * .18, 1);
  shine.position.set(-width * .5, height * .22, .006);
  shine.material.opacity = .36;

  root.add(shadow, back, fill, shine);
  root.userData.parts = { shadow, back, fill, shine };
  root.userData.width = width;
  root.userData.height = height;
  return root;
}

export default class CombatVisualDirectorV110 {
  ...
  hidePrototypeVisuals(group) {
    ...
    group?.traverse?.((object) => {
      if (!object.isMesh || object.userData?.combatVisualV110) return;
      if (preserved.has(object) || PRESERVE_NAME.test(object.name || '')) return;
      object.userData.combatVisualPreviousVisibleV110 = object.visible;
      object.visible = false;
      object.userData.combatVisualHiddenV110 = true;
    });
  }

  createAtlasSprite(texture, { category, assetId, scale, y, opacity = 1 } = {}) {
    ...
    sprite.userData.combatVisualV110 = true;
  ...
  createActionAura(scale, category) {
    ...
    const sprite = new THREE.Sprite(material);
    sprite.name = 'combatActionAuraV110';
  ...
  attach(group, assetId, {
    ...
    const sprite = this.createAtlasSprite(texture, { category, assetId, scale, y });
    ...
    group.userData.combatVisualMode = 'directional-action-projection-v110';
    this.attachments += 1;
  ...
  attachEnemy(group, type = 'imp', config = {}, options = {}) {
    ...
    const assetId = table[type] || fallback;
  ...
  attachCitadel(core, options = {}) {
    if (!core || core.userData?.guardianCitadelV110) return Boolean(core?.userData?.guardianCitadelV110);
    ...
    const sprite = new THREE.Sprite(material);
    sprite.name = 'guardianCitadelV110';
    ...
    core.userData.combatVisualMode = 'guardian-citadel-v110';
  ...
  bindActor(group, { animation = null, getHp, getMaxHp, healthY } = {}) {
  ...
  restoreVisibility(group) {
    ...
    if (!record) return false;
    group.traverse?.((object) => {
      if (object.userData?.combatVisualHiddenV110) object.visible = false;
    });
  ...
  setDirectionalState(record, frame, state) {
    ...
    if (map) {
      map.repeat.x = side < 0 ? -1 : 1;
      map.offset.x = side < 0 ? 1 : 0;
      map.needsUpdate = true;
    }
  ...
  updateHealth(record, camera, showHealth = true) {
    ...
    const { fill, shine } = bar.userData.parts;
  ...
  updateRecord(record, dt, camera, elapsed, showHealth = true) {
    ...
    const perspectiveX = 1 - directionTurn * .30;
    const rearShade = 1 - directionTurn * .22;
    record.sprite.position.x = directionSide * directionTurn * record.baseScale * .045;
  ...
  get diagnostics() {
    ...
    return Object.freeze({
      mode: 'directional-action-projection-v110',
...
export { DIRECTIONS as COMBAT_DIRECTIONS_V110, STATES as COMBAT_ACTION_STATES_V110 };
