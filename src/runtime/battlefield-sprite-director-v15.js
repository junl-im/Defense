...
export default class BattlefieldSpriteDirector {
  ...
  async preload() {
    ...
    try {
      const key = this.resolution === '2x' ? 'webp2x' : 'webp1x';
      this.pageTextures = await Promise.all(IP_V15_ATLAS_PAGES.map((page) => loadTexture(page[key])));
      this.loaded = this.pageTextures.length === IP_V15_ATLAS_PAGES.length;
    } catch (error) {
      this.failed = true;
      console.warn('[BattlefieldSpriteDirectorV15] atlas preload failed', error);
    }
    return this.loaded;
  }
  ...
  createSprite(key, definition = {}) {
    ...
    const material = new THREE.SpriteMaterial({
      ...
      depthWrite: false,
      alphaTest: .035,
      opacity: Number(definition.opacity ?? (pulse ? .92 : 1)),
      toneMapped: !additive,
    ...
    sprite.renderOrder = pulse ? 6 : 2;
    sprite.userData.atlasKey = key;
    sprite.userData.baseScale = scale;
    sprite.userData.phase = angle * 2.7;
  ...
  populate(parent, { titleMode = false } = {}) {
    ...
    this.root.name = 'RuntimeAtlasBattlefieldPropsV15';
