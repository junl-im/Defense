...
export function buildRunResultPresentationV149(context = {}) {
  ...
  const finalScore = Number(score) + (won ? 5000 + Math.round((Number(coreHp) || 0) * 30) : 0);
  const summary = {};
  ...
  const unitsHtml = Object.entries(summary).map(([type, rank]) => {
    ...
    return `<span class="result-unit">${spec.symbol} ${spec.name} ${stars(rank)}</span>`;
  ...
  const analysisHtml = `
      <div><span>최고 피해</span><b>${topSpec && topDamage > 0 ? `${topSpec.symbol} ${topSpec.name}` : '대장 깨비'}</b><small>${number(topDamage || runStats.heroDamage)} 피해</small></div>
      ...
      <div><span>원정 기록</span><b>${activeRunMode.icon || '◆'} ${activeRunMode.name || '기본 원정'}</b><small>도전 ${number(runStats.trialsCompleted)}회 · 유물 ${number(runStats.relicsChosen)}개 · 세트 ${number(runStats.relicSetsActivated)}</small></div>
      ...
      <div><span>수호 의회</span><b>${councilBond.icon} ${councilBond.name}</b><small>${councilSupport.name} · 캠페인 ACT ${number(runStats.actsCleared)}/4</small></div>
      ...
      <div><span>원정 시드</span><b>${String(runSeed || '-')}</b><small>${dailyEdict.icon || '◆'} ${dailyEdict.name || '기본 칙령'} · ${selectedSeedModeId === 'daily' ? '오늘의 원정' : '자유 원정'}</small></div>`;
