...
export class ProductionConsole {
  ...
  render() {
    ...
    this.element.innerHTML = `
      <header><div><small>DD PRODUCTION OS</small><b>VISUAL POLISH 1.0.12 · 147,232 KNOWLEDGE RECORDS</b></div><button type="button" data-close-console aria-label="제작 콘솔 닫기">×</button></header>
      <section class="production-console-grid">
        <article><small>RELEASE</small><b>v${versionPolicy.releaseVersion || '1.0.12'} · ${versionPolicy.buildId || 'b24.12'}</b><span>Legacy ${versionPolicy.lineageVersion || '23.1.0'} · ${versionPolicy.id || 'version policy'}</span></article>
