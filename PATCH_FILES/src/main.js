...
class DokkaebiLuckDefense {
  ...
  damageEnemy(enemy,amount,source='',hitOrigin=null) {
    ...
    let shielded = false;
    if (enemy.type === 'brute' && hitOrigin && source !== 'skill') {
  ...
  animate() {
    ...
    if(this.state==='playing') {
      this.updatePlayer(dt);this.updateWave(dt);this.updateEnemies(dt);this.updateHazards(dt);this.updateUnits(dt);this.updateProjectiles(dt);this.updateCoins(dt);this.updateParticles(dt);this.updateKillChain(dt);this.updateAdaptiveQuality(dt);this.updateHUD();
